      ...
      <div className="mx-auto max-w-[1400px] px-6 py-20 lg:px-10">
        <div className="grid gap-14 lg:grid-cols-[1.4fr_2fr]">
            ...
            <div className="mt-6 flex gap-3">
              {["IG", "X", "IN", "YT"].map((s) => (
                <a
                  key={s}
                  href="#"
                  className="flex h-10 w-10 items-center justify-center rounded-full border border-black/10 text-xs font-semibold text-gray-muted transition-colors hover:border-primary hover:text-primary"
                >
                  {s}
                </a>
              ))}
          ...
          <div className="grid grid-cols-2 gap-8 sm:grid-cols-4">
            {columns.map((col) => (
              <div key={col.title}>
                <div className="mb-4 text-xs font-semibold uppercase tracking-wider text-gray-muted">
                  {col.title}
                </div>
                <div className="flex flex-col gap-3">
                  {col.links.map(([label, href]) => (
                    <Link key={label} href={href} className="text-sm text-black/70 hover:text-primary">
                      {label}
                    </Link>
                  ))}
                </div>
