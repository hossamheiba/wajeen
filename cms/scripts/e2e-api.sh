...
#
# Its own database, seeded from the repository JSON and from the images under
# public/, so a test run can publish, roll back and rebind media without
# touching the development data. The repository files are only ever read.
set -euo pipefail
...
# the one Playwright gives the Next server, or every submission is a 403.
export WJEEN_INQUIRY_TOKEN="${WJEEN_INQUIRY_TOKEN:-e2e-inquiry-token}"
# CVs land in a throwaway directory, never under MEDIA_ROOT.
export WJEEN_PRIVATE_ROOT="${WJEEN_PRIVATE_ROOT:-$PWD/privatefiles-e2e}"
...
# The media library, seeded from the same public/images the site still ships.
# Without it the site's images all fall back, and the tests that prove the
# CMS-managed path works would pass for the wrong reason.
